...
class boostprinter(object):
    ...
    def line(self, line_xy):
        ...
        if line_xy[1] == 0 and line_xy[2] == 0:
            ...
            if line_xy[1] < 0:
                direction_x = -1
            time_x = line_xy[1] * self.BASE_TIMED / 3
            ...
            time_y = line_xy[2] * self.BASE_TIMED / 3
            direction_y = 1
            if line_xy[2] < 0:
                direction_y = -1
            ...
            if abs(line_xy[1]) > abs(line_xy[2]):
                ...
                speed_x = self.X_BASE_SPEED * ratio * direction_x
                speed_y = self.Y_BASE_SPEED * direction_y
                time_xy = line_xy[2] * self.BASE_TIMED / 3
            else:
                ratio = (abs(line_xy[2]) / abs(line_xy[1]))
                speed_y = self.Y_BASE_SPEED * ratio * direction_y
                speed_x = self.X_BASE_SPEED * direction_x
                time_xy = line_xy[1] * self.BASE_TIMED / 3
